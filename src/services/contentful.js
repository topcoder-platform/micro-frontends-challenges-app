...
class Service {
  ...
  async getEDUTaxonomy() {
    ...
    taxonomy = _.groupBy(
      taxonomy.items.map((item) => ({ ...item.fields, id: item.sys.id })),
      "trackParent"
    );
    return taxonomy;
  }
  ...
  async getEDUContent({
    ...
    if (tax && track && taxonomy && taxonomy[track]) {
      ...
      if (taxIDs.length)
        query["fields.contentCategory.sys.id[in]"] = taxIDs.join(",");
    }
    ...
    if (sortBy) {
      switch (sortBy) {
        case "Likes":
          query.order = "-fields.upvotes,-fields.creationDate";
          break;
        default:
          query.order = "-fields.creationDate";
          break;
      }
    }
    const content = {};
    await Promise.all(
      _.map(types || EDU_ARTICLE_TYPES, (type) =>
        this.queryEntries({ ...query, "fields.type": type })
