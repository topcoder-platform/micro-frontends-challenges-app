  ...
  if (opportunities) {
    ...
    _.map(opportunities, (opportunity) => {
      ...
      if (challenge.technologies) {
        ...
        challenge.subTrack === OLD_SUBTRACKS.TEST_SUITES ||
...
class ReviewOpportunitiesService {
  ...
  submitApplications(challengeId, roleIds) {
    ...
    return this.private.api.post(endpoint, {}).then((res) => JSON.parse(res));
  }
  ...
  cancelApplications(challengeId, roleIds) {
    ...
    return this.private.api.delete(endpoint, {}).then((res) => JSON.parse(res));
  }
}
