...
export function normalizeChallenge(challenge, username) {
  ...
  if (registration) {
    registrationStartDate =
      registration.actualStartDate || registration.scheduledStartDate;
    if (registration.isOpen) {
      registrationOpen = "Yes";
    }
    registrationEndDate =
      registration.actualEndDate || registration.scheduledEndDate;
  }
  const groups = {};
  if (challenge.groups) {
...
class ChallengesService {
  ...
  constructor(tokenV3, tokenV2) {
     ...
     *  `filter`.
     */
    const getChallenges = async (endpoint, filter) => {
      let res = {};
      if (
        _.some(filter.frontFilter.tracks, (val) => val) &&
    ...
    const getChallengeDetails = async (endpoint, legacyInfo) => {
      let query = "";
      if (legacyInfo) {
     ...
     *  `filter`.
     */
    const getMemberChallenges = async (endpoint, filters = {}, params = {}) => {
      ...
      const url = `${endpoint}?${qs.stringify(
        _.omit(query, ["limit", "offset", "technologies"])
      )}`;
  ...
  async activate(challengeId) {
    ...
    let res = await this.private.apiV5.patch(
  ...
  async createTask(
    ...
    const registrationPhase = await this.private.apiV5.get(
      "/challenge-phases?name=Registration"
    );
  ...
  async getFullChallengeDetails(challengeId) {
    ...
    if (challenge) {
      ...
      if (memberId) {
        ...
        submissions = await this.private.submissionsService.getSubmissions(
          subParams
        );
        ...
        if (submissions) {
          ...
          submissions.forEach((s, i) => {
            submissions[i].review = _.reject(
              s.review,
              (r) => r && _.includes(reviewScans, r.typeId)
            );
          });
  ...
  async getChallengeRegistrants(challengeId) {
    ...
    const roleId = this.private.tokenV3
      ? await this.getRoleId("Submitter")
      : "";
    const params = {
      challengeId,
    };
  ...
  async getChallengeTypeId(abbreviation) {
    const ret = await this.private.apiV5
      .get(`/challenge-types?abbreviation=${abbreviation}`)
      .then(checkErrorV5)
      .then((res) => res);
  ...
  async getChallenges(filter) {
    return this.private.getChallenges("/challenges/", filter).then((res) => {
  ...
  async getRecommendedChallenges(filter, handle) {
    ...
    const challenges = res.result
      ? res.result.filter((ch) => ch.jaccard_index > 0)
      : [];
    return {
      challenges,
      totalCount,
  ...
  async getUserChallenges(userId, filters, params) {
    ...
    ChallengesService.updateFiltersParamsForGettingMemberChallenges(
      userFilters,
      params
    );
    ...
    const url = `/challenges?${qs.stringify(
      _.omit(query, ["limit", "offset", "technologies"])
    )}`;
    const userChallenges = await this.private.apiV5
      .get(url)
      .then(checkErrorV5)
      .then((res) => {
  ...
  async getUserResources(userId, page = 1, perPage = 1000) {
    const res = await this.private.apiV5.get(
      `/resources/${userId}/challenges?page=${page}&perPage=${perPage}`
    );
    return res.json();
  }
  ...
  async getUserMarathonMatches(memberId, filter, params) {
    ...
    const res = await this.private.apiV5.get(
      `/challenges?${qs.stringify(newParams)}`
    );
    return res.json();
  }
  ...
  async getRoleId(roleName) {
    ...
    const roles = await this.private.apiV5
  ...
  async unregister(challengeId) {
    ...
    const res = await this.private.apiV5.delete(
      "/resources",
  ...
  getActiveChallengesCount(handle) {
    ...
    return this.getUserChallenges(handle, filter, params).then(
      (res) => res.totalCount
    );
  }
      ...
      .then(
        (res) => {
          ...
          return api
            ...
            .then((procres) => JSON.parse(procres));
        },
        (err) => {
          logger.error(
            `Failed to submit to the challenge #${challengeId}`,
            err
          );
          throw err;
        }
  ...
  async getUserRolesInChallenge(challengeId) {
    ...
    if (resources)
      return _.map(
        _.filter(resources, (r) => r.memberHandle === user.handle),
        "roleId"
      );
