...
function getTermsDone(entity, tokens, mockAgreed) {
  ...
  const mockAgreedArray = mockAgreed
    ? Array(10 + 1)
        .join("1")
        .split("")
        .map(() => true)
  ...
  switch (entity.type) {
    ...
    case "community": {
      termsPromise = service.getCommunityTerms(
        entity.id,
        tokens.tokenV3,
        mockAgreedArray
      );
    ...
    case "reviewOpportunity": {
      termsPromise = service.getReviewOpportunityTerms(
        entity.reviewOpportunityTerms
      );
      break;
    }
    default:
      throw new Error(
        `Entity type '${entity.type}' is not supported by getTermsDone.`
      );
  }
...
function agreeTermDone(termId, tokenV3) {
  ...
  return service
    .agreeTerm(termId)
    .then((resp) => ({ termId, success: resp.success }));
}
...
function checkStatusDone(entity, tokens) {
   ...
   */
  const checkStatus = (maxAttempts) =>
    getTermsDone(entity, tokens, mockAgreed).then((res) => {
