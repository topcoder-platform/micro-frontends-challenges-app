...
export default class EditorWrapper extends React.Component {
  ...
  applyColorStyle(type, color) {
    ...
    contentState = _.reduce(
      EDITOR_COLOR_MAP,
      (state, value, name) =>
        Modifier.removeInlineStyle(state, sel, `${type}_${name}`),
      contentState
    );
  ...
  insertImage(src, triggerModal) {
    ...
    contentState = contentState.createEntity("IMG", "SEGMENTED", {
      src,
      triggerModal,
    });
    ...
    contentState = Modifier.insertText(contentState, sel, " ", null, key);
  ...
  insertLink(title, href, triggerPopup) {
    ...
    if (sel.isCollapsed()) {
      ...
      contentState = Modifier.insertText(contentState, sel, " ", null, null);
      // Because selection hasn't been updated, this will insert the link *before*
      // the newly created space.
      contentState = Modifier.insertText(contentState, sel, title, null, key);
      ...
      // Selection is a range, keep the text but make it a link
    ...
    return (
      ...
      >
        <Editor
          ...
          plugins={[st.markdown ? this.markdownPlugin : {}, this.customPlugin]}
          ref={(node) => {
            this.node = node;
          }}
          spellCheck
