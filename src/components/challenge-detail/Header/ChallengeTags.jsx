    ...
    <div>
      {challengeType && (
        <TrackTag
          onClick={() =>
            setImmediate(() =>
              setChallengeListingFilter({ types: [challengeType.abbreviation] })
            )
          }
        ...
        (tag) =>
          tag && (
            <Tag
              key={tag}
              onClick={() =>
                setImmediate(() => setChallengeListingFilter({ search: tag }))
              }
              to={`${challengesUrl}?search=${encodeURIComponent(tag)}`}
            >
              {tag}
            </Tag>
          )
