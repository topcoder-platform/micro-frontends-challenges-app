...
export const BUCKET_DATA = {
  [BUCKETS.ALL]: {
    ...
    // hideCount: false,
    name: "All Challenges",
    sorts: [SORTS.MOST_RECENT_START_DATE, SORTS.TITLE_A_TO_Z],
  },
  [BUCKETS.MY]: {
    // filter: {
    //   status: ['Active'],
...
export const NO_LIVE_CHALLENGES_CONFIG = {
  ...
  [BUCKETS.OPEN_FOR_REGISTRATION]:
    "No challenges found in Open for Registration Challenges",
...
export function filterChanged(filter, prevFilter) {
  ...
  return (
    !_.isEqual(filter.tracks, prevFilter.tracks) ||
    ...
    _.filter(filter.tags, (val) => _.indexOf(prevFilter.tags, val) < 0).length >
      0 ||
    _.filter(prevFilter.tags, (val) => _.indexOf(filter.tags, val) < 0).length >
      0 ||
    _.filter(filter.types, (val) => _.indexOf(prevFilter.types, val) < 0)
      .length > 0 ||
    _.filter(prevFilter.types, (val) => _.indexOf(filter.types, val) < 0)
      .length > 0
  );
}
...
export function sortChangedBucket(sorts, prevSorts) {
  ...
  if (sorts.openForRegistration !== prevSorts.openForRegistration)
